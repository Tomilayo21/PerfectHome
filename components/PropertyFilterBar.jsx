...
export default function PropertyFilterBar() {
  ...
  const triggerClasses =
    "flex items-center justify-between text-black dark:text-black w-full sm:w-40 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-black bg-white";
  const itemClasses =
    "text-sm text-black dark:text-black px-3 py-2 rounded-md cursor-pointer hover:bg-gray-100 data-[state=checked]:bg-gray-200";
        ...
        {summaryChips.length > 0 && (
          <>
            ...
            {summaryChips.map((chip, idx) => (
              <span key={idx} className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-full">
                {chip.label}
                <button
                  onClick={() =>
                    chip.name === "price"
            ...
            <button onClick={() => router.push(window.location.pathname)} className="ml-2 text-xs md:text-sm text-gray-600 hover:text-gray-800 font-medium underline">
              Clear all
            </button>
          </>
        )}
