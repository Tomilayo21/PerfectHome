...
const HeroSection = () => {
  ...
  useEffect(() => {
    const loadHeroVideo = async () => {
      ...
      console.log("HERO VIDEO FROM API:", data); // ← Check structure

      if (data.heroVideo) {
        setHeroVideo(data.heroVideo);
      } else {
        // fallback if API returns videoUrl directly
        setHeroVideo(data);
      }
    };
  ...
  return (
    <section
      className="relative w-full flex flex-col items-center justify-center overflow-hidden text-white"
      style={{ height: heroVideo.height }}
    >
      {/* === Background Video === */}
      <video
        key={heroVideo.videoUrl}   // 👈 forces reload when URL updates
        autoPlay
        loop
        muted
        playsInline
      ...
      >
        <source
          src={
            heroVideo.videoUrl && heroVideo.videoUrl.length > 5
              ? heroVideo.videoUrl
              : "/VID-20251105-WA0027.mp4"
          }
          type="video/mp4"
        />
      </video>
